...
The app evolved from the drag-drop editor into **Spell Studio** (entry: `src/main.jsx`, which renders `StudioPage` directly — wrapped in `ThemeProvider → AuthProvider`). **There is no `router.jsx` and no browser routes.** Admin is reached through a capability-gated overlay: `ConfigPanel` (visible when `role === 'admin'`, gated by `src/app/capabilities.js`) lazy-loads `AdminOverlay`, which wraps `AdminPage` in a `MemoryRouter` so admin navigation works without touching the browser URL. Login lives inside `ConfigPanel`, not on a separate route. **The legacy drag-drop editor (`App.jsx` + `components/{GlyphCanvas,Inspector,SpellTree,InkPanel,Palette}`, `DrawModal`) was removed** — only `components/ResultPanel.jsx` remains from the old UI.

> **Layered `src/` (architecture refactor, 2026-06).** The code is organized into layers — `src/domain/` (pure engine + recognizer logic: no React/JSON/DOM/Supabase), `src/services/` (I/O + JSON/DB binding: `supabase/`, `ai/`, `recognizer/`, `symbols/`), `src/features/` (`studio/`, `admin/`), and `src/shared/` (`theme/`, `capabilities/`, `ui/`, `auth/`). Cross-layer imports use the `#domain` / `#services` / `#shared` / `#features` aliases (package.json `imports` + Vite). The old paths below (`src/studio/`, `src/draw/`, `src/engine/`, `src/ai/`, `src/data-services/`, `src/app/`, `src/theme/`, `src/components/`) are **transitional re-export shims** kept so existing references resolve; the canonical sources live in the layers. The `domain/`+`services/` purity boundary is ESLint-enforced. See [docs/specs/current-architecture-spec.md](docs/specs/current-architecture-spec.md) and [docs/specs/refactor-plan-spec.md](docs/specs/refactor-plan-spec.md). Paths in the bullets below name the *role*; resolve them to the layer location.

- **Studio** (`src/features/studio/`): a near-fullscreen **react-konva** canvas (`DrawingSurface.jsx`) with paint tools (brush/line/rect/triangle/circle/arrow, stroke + pixel erasers, select/move/rotate, area-select), **right-drag/space pan + Recenter**, Shift+wheel zoom, **undo/redo (Ctrl+Z/Y)**, and **tool keyboard shortcuts** (B/L/R/G/C/A/V/M/T/E) + dye colors; place registered sigils/signs from `SymbolPalette.jsx`. **Detect** runs the `$P` recognizer (`src/draw/recognizer.js`) over the drawn strokes (+ placed symbols) → a `composition`; detections are weighted by training `source` and gated by a **confidence threshold** (low-confidence reads "unknown?" and is kept out of the engine input), shown as correctable overlay boxes. **Analyze** runs the engine + a streaming multi-topic **AI report** (`src/ai/report.js`), and logs the analysis + corrections via `data-services/analyses.js` (the improvement loop). Copy-image + JSON export/import via the `DrawingSurface` ref. The resizable, state-persisting results drawer holds it all. Corrections and confident catalog matches feed the training set.
...
Plans/specs live in **[docs/app/](docs/app/)**: [APP-PLAN.md](docs/app/APP-PLAN.md) (master), [SPEC.md](docs/app/SPEC.md) (workstreams WS0–WS11), [DRAWING-APP.md](docs/app/DRAWING-APP.md). Manual test plan: [TEST-PLAN.md](docs/app/TEST-PLAN.md). Improvement backlog: [IMPROVEMENTS.md](docs/app/IMPROVEMENTS.md). **The per-feature specs live in [docs/app/specs/](docs/app/specs/)** — *active/partial* ones at the top level ([SPEC-recognizer-analysis.md](docs/app/specs/SPEC-recognizer-analysis.md) — training flywheel + analysis surfacing, core shipped, A7 ML + B5 cluster deferred; [SPEC-cluster-recognition.md](docs/app/specs/SPEC-cluster-recognition.md) — Phase 1 partial) — and *shipped/superseded* specs archived under [docs/app/specs/done/](docs/app/specs/done/) (symbol-versioning, sign-variants-sizing/magnitude-and-variants, spell-ir, visual-renderer, ring-closure-floodfill, recognizer-matching, admin-verify, ai-report-surfacing, nested-linked, tuning-tools, symbol-roles, stroke-beautify, the two studio-ux-cleanups). Each spec carries an updated `Status:` header. (The magic-system source + reasoning docs stay in `docs/`.)
...
# symbol lifecycle: flag catalog spells whose symbols changed (docs/app/specs/done/SPEC-symbol-versioning.md):
...
- `inverted` flips the glyph top↔bottom (negate Y) and, for invertible signs, negates the operator's effect; `mirrored` flips it left↔right (negate X) and is **visual-only** (ignored by the engine). The Studio renders `inverted` live on placed symbols in [DrawingSurface.jsx](src/studio/DrawingSurface.jsx) (negated `scaleY`); the SVG export renderer [render.mjs](tools/render.mjs) honors both. (The legacy editor's selection-toolbar flip toggles went away with `App.jsx`.)
...
- **Sign size affects direction** (Layer 1 of docs/app/specs/done/SPEC-sign-variants-sizing.md): `computeOrientationAim` weights each directional sign's facing by its `scale` (or `metrics.directionalMagnitude`) — equal opposing signs cancel, a larger one wins (the Column-"T" lesson). Variant *metrics* (e.g. stem length) and ring/sigil sizing are later layers.
