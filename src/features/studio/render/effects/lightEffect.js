// lightEffect.js — Light element particle effect (trail-steered beam particles).
// PURE MODULE — no JSON imports. Config injected via the `config` argument.
// Ported and adapted from wha-spell-simulator/src/renderer/effects/lightEffect.js.
...
function traceLightTrail(ctx, trail, flow, phase) {
  ...
  for (let index = 1; index < projectedTrail.length - 1; index++) {
