// fireEffect.js — Fire element particle effect.
// PURE MODULE — no JSON imports. Config injected via the `config` argument.
// Ported and adapted from wha-spell-simulator/src/renderer/effects/fireEffect.js.
...
export function fireFlowConfig(spellIR, ring, portal, frame) {
...
function drawFireParticle(ctx, particle, flow, spellIR, opacity) {
  ...
  const gradient = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, displayRadius * (1.2 - alpha * 0.2))
