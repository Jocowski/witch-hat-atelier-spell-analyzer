...
**Closest canon analogue — the Warmth-Retention Seal.** Both are gentle fire-family heat spells (the Warmth-Retention Seal also uses a fire sigil, [sigils.md:15](../sigils.md#L15)). The difference, from the names and uses: the **Snugstone *generates and radiates*** warmth (an active heat source: Radial-tempered fire + omnidirectional dispersal), while a **Warmth-Retention** seal *insulates / holds existing heat in*. Same element, opposite strategy — produce vs. preserve. (Comparison drawn from the docs, not an engine catalog match — the catalog is empty.)
