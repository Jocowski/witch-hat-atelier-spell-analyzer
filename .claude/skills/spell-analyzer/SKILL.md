...
- 2026-06-01: **Rising Wave (CANON, unofficial name) — half-ring Region placement = diagonal propulsion; the engine MISSES it (reads "contained").** Water core + 8 Column (full ring) + 4 Region on ONE HALF, interspersed = a surge of water that travels up **diagonally**, propelling the caster into the air (Qifrey vs. Sasaran, Serpentback Cave). **Key engine limitation:** `classifyRegion` only inspects Region *orientation*. All 4 Regions face inward (each rotation = posAngle+180), so the engine fits the canon "all inward ⇒ contained within the ring" config and reports `aim=contained` — WRONG. The real effect is diagonal because the Regions cover only a **semicircle**; partial-ring coverage is a *fifth* config the docs don't enumerate and the engine doesn't model (it ignores WHERE the Regions sit, only HOW they face). Don't trust engine `aim` when directional signs are clustered on part of the ring — read the positional distribution yourself. Element-physics: water *creates* its material (sigils.md:41) so propulsion works on dry ground (self-sufficient); leans on costly create/burst mode for volume. Closest analogue = Rising Platform of Water (lifts straight up via Levitation, signs.md:55) vs Rising Wave (launches diagonally via Region asymmetry) — same goal, opposite mechanism. Added to catalog (id `rising_wave`, confidence high; self-match 0.95). Doc: docs/spells/Rising_Wave.md. **Also: user asked to STOP repeating the "ring open/closed is just a visual" disclaimer in analyses — omit active/inactive silently (memory: no-ring-visual-disclaimer).** **ENGINE NOW FIXED:** added `computeRegionCoverage(signs)` (positional resultant of WHERE region signs sit) + a `biased` branch in `classifyRegion` (geometry.js): inward-facing regions that cover only one arc (coverage magnitude > 0.34) now classify as `{mode:'biased', angle: centroidAngle}` instead of `inward`/contained. Wired the `biased` label into compose.js (`aim = "surging <dir> (uneven region ring)"`) and deduce.js (summary `, surging <dir> (the region signs ring only one side of the seal)`). Balanced full rings stay `inward` (regression-tested). Rising Wave now reads aim=**surging up** (was "contained"); centroid is straight up because the regions are symmetric about vertical — tilting the cluster off-vertical yields the canon diagonal. Tests in test/geometry.test.js (43 pass). Element-agnostic; canon only demonstrates the surge for water.
- 2026-06-01: **Rising Platform of Water (CANON) — water/fire/light levitation LIFTS WITHOUT STEERING; only air/wind levitation is directional (signs.md:55). Don't call non-uniform levitation angles "wrong" on a water spell.** Water core + 4 Column (outward) + 4 Levitation (drawn horizontal) alternating = a column of water that lifts the object the seal is on straight up (Richeh's master used it to cross the Dadah Range floating mountains; Second Pentagram Test arc). **Big lesson — I initially over-flagged the user's levitation rotations as "wrong."** For WATER, levitation only lifts; the arrow's *direction does not steer the effect* (signs.md:55 — direction only matters for air/wind). So non-uniform/varied levitation angles on a water spell are **functionally free** (cosmetic), NOT a defect — the lift is vertical regardless. The engine, however, routes EVERY directional sign (incl. levitation) through directional-aim/spin math **element-agnostically**, so non-uniform water-levitation makes it report a spurious lateral "aim" (e.g. "down-right") and/or "spin." Treat those as engine artifacts on water/fire/light levitation spells, not real skew. Clean version (Columns in NORMAL orientation = longer line inward / `inwardRotation`=posAngle+180, Levitations uniform horizontal/tangential=posAngle±90) reads aim=**above the seal** (straight up); a uniformly-tangential levitation ring still trips the "spin" note, which is also spurious for water. Engine nearest = Water Orb (0.617). Added to catalog (id `rising_platform_of_water`, confidence high). Doc: docs/spells/Rising_Platform_of_Water.md. **Second lesson (column orientation):** I *also* mis-flipped the columns to outward, trusting a pasted "outward-facing column signs" description over the art. For a column the NORMAL canon orientation is **longer line inward** (the short ⊤ cap faces outward) — signs.md:34 / signs.json:25 "shorter line usually faces outward" — which is what the user's original export already had. In the engine, inward-vs-outward column rotation is **cosmetic** (radially symmetric ring fires straight up either way; deduced effect identical). What actually changes a column is the **`inverted` flag** (invertedVerb: "driven inward and erupts… ≈ dispersion", signs.md:34), NOT its rotation. Don't conflate "rotated to face outward" with "inverted."
- 2026-06-01: **Rainbringer Seal (CANON) — the textbook Rain spell; one Rain sign IS the whole form (it's the surround, not a ringed element).** Water core + a single **Rain** sign surrounding it = a localized downpour ("The water falls as rain across the surrounding area"). Key canon points: Rain "is meant to surround the central sigil" (signs.md:287), so ONE large Rain is canon-correct — don't ring multiples. The engine's "potentially unstable (asymmetric)" flag is a single-sign geometry artifact (one sign read slightly off origin), NOT a real defect — note this rather than reporting instability. Rain is semi-directional/invertible: upright = rainfall, inverted ≈ draw-in/drying (the Rainflinger "Drying" mode, signs.md:289 / spells.md:81). Element-physics: cleanest sigil/sign pairing in the water set — water is a fluid that naturally breaks into falling droplets, exactly Rain's mechanic; and water can *create* (burst) or the cheaper *collect* (sustained, sigils.md:41), so a Rainbringer is self-sufficient where any other element would need a reservoir. Never seen cast on-page (mural-only, Serpentback Cave arc). Added to catalog (id `rainbringer_seal`); self-matches 0.85. Doc: docs/spells/Rainbringer_Seal.md.
